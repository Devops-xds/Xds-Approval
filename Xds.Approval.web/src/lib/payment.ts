export type PaymentType = 'Goods' | 'Service' | 'Residence' | 'Non-Residence' | 'Crossboarder';
...
export const getPaymentTypeLabel = (paymentType?: string | null) => {
  ...
  if (normalized === 'non-residence' || normalized === 'non residence' || normalized === 'nonresidence') return 'Non-Residence';
...
export const getTaxProfile = (paymentType?: string | null): TaxProfile => {
  ...
  switch (normalized) {
    ...
    case 'service':
      return { label: 'Service', whtRate: 0.075 };
    case 'residence':
      return { label: 'Residence', whtRate: 0.1 };
    case 'non-residence':
      return { label: 'Non-Residence', whtRate: 0.15 };
    case 'crossboarder':
